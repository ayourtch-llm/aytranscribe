...
use candle_nn::{
    Activation, Embedding, Linear, Module, VarBuilder, embedding, linear, linear_no_bias,
};
use std::sync::Arc;
...
impl Module for RmsNorm {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let norm_xs = xs.broadcast_div(&((xs.sqr()?.sum_keepdim(D::Minus1)? / xs.dim(D::Minus1)? as f64)?
            + self.eps)?
            .sqrt()?)?;
        norm_xs.broadcast_mul(&self.weight)
    }
}
...
impl Model {
    ...
    fn prepare_attention_mask(&self, attn_mask: &Tensor) -> Result<Tensor> {
        ...
        let mut masks: Vec<Tensor> = vec![];
        for b in 0..b_sz {
            masks.push(attn_mask.i((b, ..))?.expand((1, 1, sql_len, sql_len))?);
        }
...
impl ModelForCausalLM {
    ...
    pub fn forward_embeds(
        ...
        let (_b_size, seq_len, _) = input_embeds.dims3()?;
        self.base_model
            .forward_embeds(input_embeds, seqlen_offset, attn_mask)?
            .narrow(1, seq_len - 1, 1)?
            .apply(&self.lm_head)
    }
...
fn repeat_kv(xs: Tensor, num_kv_groups: usize) -> Result<Tensor> {
    if num_kv_groups == 1 {
        return Ok(xs);
    }
    let (b_sz, n_kv_head, seq_len, head_dim) = xs.dims4()?;
    xs.unsqueeze(2)?
        .expand((b_sz, n_kv_head, num_kv_groups, seq_len, head_dim))?
        .reshape((b_sz, n_kv_head * num_kv_groups, seq_len, head_dim))
}
...
mod tests {
    ...
    #[test]
    fn model_supports_inputs_embeds() {
        let vm = VarMap::new();
